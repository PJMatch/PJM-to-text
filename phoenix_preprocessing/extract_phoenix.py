...
"""
X were local paths
phoenix_root was download Phoenix 14T dataset
output_root was temporary storage
later on the .npy files were uploaded to google drive in order not to take git memory storage
"""

PHOENIX_ROOT = Path(r'X') 
OUTPUT_ROOT = Path(r'X')
...
def main():
    """
    Main execution loop: Iterates through dataset splits ('train', 'dev', 'test'),
    processes frames, and saves the extracted keypoints maintaining the directory structure.
    """
    ...
    # Global timestamp must increase throughout every dataset split
    ...
    for split in dataset_splits:
        ...
        # Verify if the split directory exists before processing
        if not split_dir.exists():
            print(f"Warning: Directory {split_dir} not found. Skipping {split} split.")
            continue
        ...
        for folder in tqdm(subfolders, desc=f"Processing {split.upper()} set"):
            ...
            np.save(output_split_dir / f"{seq_name}.npy", np.array(sequence_data))

    pose_detector.close()
    hand_detector.close()
    face_detector.close()
    print("\nFull dataset preprocessing completed successfully.")
