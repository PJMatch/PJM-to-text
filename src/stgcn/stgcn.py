...
class STGCNCoSign1s(nn.Module):
    ...
    def __init__(self, config_path=None):
        ...
        # Per-group BatchNorm for confidence-aware centralization (as in mmpose-trainingv2)
    ...
    def forward(self, x, keep_prob=0.8):
        ...
        for name in ["body", "face", "mouth", "l_hand", "r_hand"]:
            ...
            # Confidence-aware centralization as in mmpose-trainingv2
            # Only x, y are centralized; confidence (channel 2) is left untouched
            root_point = group_data[:, :2, :, 0:1]  # [B, 2, T, 1]
            centralized = group_data.clone()
            # centralization - eqn. (2)
            centralized[:, :2] = group_data[:, :2] - root_point

            # Zero low-confidence points AFTER centralization
            # so missing points stay at (0, 0) instead of wandering to -root_point
            ...
            # Per-group BatchNorm on (C * V_local) features, pooled over (B, T)
            # Input layout expected by BN1d: [B, C*V, T]
        ...
        # Chunk-based masking with τ=25 frames (matching CoSign paper as in mmpose-trainingv2)
        t_B = v_groups.size(0)
        t_N = v_groups.size(1)
        t_T = v_groups.size(3)
        tau = 25
    ...
    def _create_args(self, gso_matrix):
        return STGCNArgs(
            ...
            gso=gso_matrix,
            enable_bias=True,
            droprate=0.3,  # Changed from 0.2 to 0.3 as in mmpose-trainingv2
        )
